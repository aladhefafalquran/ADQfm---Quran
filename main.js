...
// Check if current device is an iOS/Apple device
  ...
  try {
    ...
    for (const line of lines) {
      if (line.startsWith("#EXTINF:")) {
        ...
        // For iOS devices, directly replace .ogg with .mp3 in the URL
        if (isAppleDevice && url.toLowerCase().endsWith('.ogg')) {
          url = url.replace(/\.ogg$/i, '.mp3');
        }
...
function playTrack(index, startTime = 0, isRandom = false) {
  ...
  console.log(`Playing track: ${track.name} from ${startTime}s, URL: ${track.url}`);
  ...
  function handlePlaybackError() {
    // Try alternate format if OGG format
    const originalUrl = track.url;
    if (originalUrl.toLowerCase().endsWith('.ogg')) {
    ...
    } else if (originalUrl.toLowerCase().endsWith('.mp3')) {
      // Try OGG if MP3 fails
      const oggUrl = originalUrl.replace(/\.mp3$/i, '.ogg');
      console.log("Trying OGG format:", oggUrl);
      
      audioPlayer.src = oggUrl;
      audioPlayer.load();
      audioPlayer.play().catch(err => {
        console.error("OGG format also failed:", err);
        moveToNextTrack();
      });
    } else {
      moveToNextTrack();
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  console.log("iOS device detected:", isAppleDevice);
